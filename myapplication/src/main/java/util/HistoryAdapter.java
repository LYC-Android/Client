...
public class HistoryAdapter extends RecyclerView.Adapter<HistoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        holder.mTime.setText( mDatas.get(position).getCreateTime());
        List<DatabaseInfo> databaseInfos = DataSupport.select("realName").
                where("username = ?", mDatas.get(position).getAuthor()).find(DatabaseInfo.class);
        if (databaseInfos.size() > 0) {
            holder.mName.setText("医生姓名:" + databaseInfos.get(0).getRealName());
        } else {
